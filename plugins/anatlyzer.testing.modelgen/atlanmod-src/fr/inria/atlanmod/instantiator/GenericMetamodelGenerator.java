...
public class GenericMetamodelGenerator {
	...
	public String getMetaModelResourceName() {
		URI metamodelURI = config.getMetamodelResource().getURI();
		String lastSegm = metamodelURI.lastSegment();
		if (lastSegm.contains(File.separator)) {
			return lastSegm.substring(lastSegm.lastIndexOf(File.separator)+1,lastSegm.lastIndexOf(".")); 
		}
		return metamodelURI.lastSegment().substring(0, metamodelURI.lastSegment().lastIndexOf("."));
	}
