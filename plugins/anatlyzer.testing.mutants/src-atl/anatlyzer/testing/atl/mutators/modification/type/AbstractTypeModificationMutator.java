...
public abstract class AbstractTypeModificationMutator extends AbstractMutator {
	...
	void genericTypeModification(ATLModel atlModel, Class<ToModify> ToModifyClass, String featureName, MuMetaModel[] metamodels) {
		this.genericTypeModification(atlModel, ToModifyClass, featureName, metamodels, false); 
	}
	 ...
	 */
	protected <ToModify extends LocatedElement> 
	void genericTypeModification(ATLModel wrapper, Class<ToModify> ToModifyClass, String featureName, MuMetaModel[] metamodels, boolean exactToModifyType) {		
		...
		for (ToModify object2modify : modifiable) {
			EStructuralFeature featureDefinition = object2modify.eClass().getEStructuralFeature(featureName);
			...
			if (featureDefinition!=null && featureDefinition.getUpperBound() == 1) {
				...
				if (oldFeatureValue!=null) {
					List<EObject> replacements = this.replacements(wrapper, (EObject)oldFeatureValue, metamodels);
					...
					// restore original value
					// object2modify_src.eSet(featureDefinition, oldFeatureValue);
					// jesusc: Moved inside the loop to have only one restoration point
				}
			}
		...
		// COLLECTION TYPE .........................................................................
			...
			for (String option : options) {
				if (!((EClass)pack.getEClassifier(option)).isInstance(toReplace)) {
					...
					replacements.add(newElement(collectionType));
				}
			}
		} 
		...
		// COLLECTION EXPRESSION TYPE ..............................................................
			...
			for (String option : options) {
				if (!((EClass)pack.getEClassifier(option)).isInstance(toReplace)) {
					...
					replacements.add( newElement(collectionExpType));
				}
			}
		} 
		...
		// PRIMITIVE TYPE ..........................................................................
			...
			for (String option : options) {
				if (!((EClass)pack.getEClassifier(option)).isInstance(toReplace)) {
					...
					replacements.add( newElement(primitiveType));
				}
			}
		} 
	 ...
	 */
	protected EClass getCompatibleSubclass (EClass c, MuMetaModel mm) {
	 ...
	 */
	protected EClass getCompatibleUnrelatedClass (EClass c, MuMetaModel mm) {
	 ...
	 */
	protected EClass getIncompatibleSubclass (EClass c, MuMetaModel mm) {
		List<EClass> subclasses = subclasses(c, mm);
	 ...
	 */
	protected EClass getIncompatibleUnrelatedClass (EClass c, MuMetaModel mm) {
		List<EClass> unrelatedClasses = unrelatedClasses(c, mm);
	 ...
	 * @return
	 */
	private List<EClass> unrelatedClasses (EClass c, MuMetaModel mm) {
		List<EClass> subclasses       = subclasses(c, mm);
