...
public class FilterConditionChange extends CFCP {
					...
					if (itClass instanceof EClass) {		
						...
						//	
							for (EAttribute att : ((EClass)itClass).getEAllAttributes()) {
								//
								// Moreover, we discard the old filter (in CFCP, the old filter is and-concatenaded)
								mutate(andVariablePropertyHasValue(variable, att), null/*oldFilter*/, inpattern);
								//
								//
	...
	public String getDescription() {
		return "Filter Condition Change";
	}
}
